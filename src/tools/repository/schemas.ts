...
export const GetFileInputSchema = z.object({
  ...
  endLine: z
    ...
    .positive()
    .optional()
    .describe("End line number (1-based, inclusive). Use with startLine to get a specific range."),
});
...
export const GetFileAtVersionInputSchema = z.object({
  repo: z.string().describe("Repository name (e.g., 'compact', 'midnight-js')"),
  path: z.string().describe("File path within repository"),
  version: z.string().describe("Version tag (e.g., 'v1.0.0') or branch (e.g., 'main')"),
});
...
export const UpgradeCheckInputSchema = z.object({
  ...
  currentVersion: z
    .string()
    .describe("Your current version (e.g., 'v0.29.0', '0.21.0')"),
});

export const FullRepoContextInputSchema = z.object({
  repo: z.string().describe("Repository name (e.g., 'compact', 'midnight-js')"),
  includeExamples: z.boolean().default(true).describe("Include example code snippets"),
